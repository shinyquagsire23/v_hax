...
Result payload_update(u8* firmware_version)
{
    ...
    ret = gspwn((void*)(V_CODE_LINEAR_BASE + 0x00101000 - 0x00100000), buffer, (size + 0x1f) & ~0x1f);
...
void _main()
{
    ...
    // un-init DSP so killing VVVVVV will work
    ...
    if(!ret)
    {
        ...
        ret = gspwn((void*)(V_CODE_LINEAR_BASE + 0x00101000 - 0x00100000), payload_buffer, (payload_size + 0x1f) & ~0x1f);
        svcSleepThread(300*1000*1000);
    }
    ...
    if(padGet() & BUTTON_SELECT)
    {
        ...
        while(1)
        {
            switch(state)
            {
                case 0:
                    select = 0;
                    centerString(top_framebuffer, "(v*)hax menu - SALT", 24, 400);
